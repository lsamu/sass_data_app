...
const install = function (Vue, opts = { option: false }) {
  ...
  loadComponent(Vue, opts, requireComponents, "BoxVant");
  // const requireComponentsDrag = import.meta.globEager(`./**/drag.vue`);
  // loadComponent(Vue, opts, requireComponentsDrag, "BoxVantDrag");
  if (opts.option) {
    const requireOptions = import.meta.globEager("./**/option.vue");
    loadComponent(Vue, opts, requireOptions, "BoxVantOption");
  }
};
